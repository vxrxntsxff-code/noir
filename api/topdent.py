...
def handle_callback(chat_id, data, msg_id=0):
    ...
    if parts[0] == "bk":
        if parts[1] == "start":
            ...
            # Write to TopDent sheet (demo: every confirmed booking goes here)
            try:
                ...
                if _td_token:
                    ...
                    _td_req.add_header("Content-Type", "application/json")
                    with urllib.request.urlopen(_td_req, timeout=10) as _td_resp:
                        _td_result = json.loads(_td_resp.read())
                        log.info("TOPDENT SHEET OK: %s", _td_result.get("updates",{}).get("updatedRange",""))
            except Exception as e:
                log.error("TOPDENT SHEET ERROR: %s", e)
            state_del(chat_id)
                ...
                try:
                    ...
                    if not client and chat_id_d:
                        ...
                        if redis_client:
                            ...
                            client = sheets_find_client(client_name)
                    if not client and not client_name and chat_id_d:
                        client = sheets_find_client(str(chat_id_d))
                        if client:
                            client_name = client["name"]
                    if client_name:
